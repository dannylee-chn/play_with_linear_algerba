from vector import Vector
class Matrix:

     def __init__(self,list2d):
         self._values=[ row[:] for row in list2d]

     def __repr__(self):
         return 'Matrix({})'.format(self._values)

     #几行几列
     def shape(self):
         return len(self._values),len(self._values[0])

     def row_num(self):
         return self.shape()[0]
     def col_num(self):
         return self.shape()[1]

     def size(self):
         r,c=self.shape()
         return r*c

     #获得
     def __getitem__(self,pos):
         r,c=pos
         return self._values[r][c]

     def row_vector(self,index):
        return Vector(self._values[index])

     def col_vector(self,index):
         return Vector([row[index] for row in self._values])

     #基本运算性质
     def __add__(self,another):
         assert self.shape()== another.shape(),\
             'error is adding.shape of matrix must be same'
         return  Matrix([[a+b for a,b in zip(self.row_vector(i),another.row_vector(i))]
                         for i in range(self.row_num())])

     def __sub__(self,another):
         assert self.shape()== another.shape(), 'error is adding.shape of matrix must be same'
         return Matrix([[a- b for a,b in zip(self.row_vector(i),another.row_vector(i))]
                         for i in range(self.row_num())])

     def __mul__(self,k):
         return Matrix([[a*k for a in row] for row in self._values])

     def __truediv__(self,k):
         return (1/k)*self

     def __neg__(self):
         return  -1*self

     @classmethod
     def zero(cls,r,c):
          return cls([[0]*c for _ in range(r)])

     #矩阵点乘向量
     def dot(self,another):
        if isinstance(another,Vector):
            assert self.col_num()==len(another),'error in matrix-vector multiplication'
            return Vector([[self.row_vector(i).dot(another.col_vector(j)) for j in range(self.col_num())]
                           for i in range(self.row_num())])
        if isinstance(another,Matrix):
            assert self.col_num()==another.row_num(), 'error in matrix-matrix multiplication'
            return Matrix([
                [self.row_vector(i).dot(another.col_vector(j)) for j in range(another.col_num())]
                for i in range(self.row_num())])
#矩阵的转置
     def T(self):
        return Matrix([[e for e in self.col_vector(i)] for i in range (self.col_num())])

#单位矩阵
     @classmethod
     def identity(cls,n):
         m=[[0]*n for _ in range(n)]
         for i in range(n):
             m[i][i]=1
         return cls(m)